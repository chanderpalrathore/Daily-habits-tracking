...
- **Real cross-device sync** — connect the app to a small backend (Firebase, Supabase, or your own API) and swap `localStorage` calls for network calls; the state shape (`{ habits, logs, notes }`) is already backend-friendly.
